  ...
  lines.forEach((line, lineIndex) => {
    LITERAL_COLOR_PATTERNS.forEach((pattern) => {
      ...
      for (const match of matches) {
        ...
        const colorMatch = match[0].match(
          /-(zinc|neutral|gray|slate|stone|red|orange|amber|yellow|green|emerald|teal|cyan|blue|indigo|violet|purple|pink|fuchsia|rose)-/
        )
